...
#    df = df[df['amount'] >= 25000]
    ...
    df_clean = df.drop_duplicates(subset=cols_to_consider, keep='first').copy()
    df_clean.loc[:, 'supplier'] = df_clean['supplier'].str.replace('\t', '')
    df_clean.loc[:, 'supplier'] = df_clean['supplier'].str.replace('\n', '')
    df_clean.loc[:, 'supplier'] = df_clean['supplier'].str.replace('\r', '')
    df_clean.loc[:, 'supplier_upper'] = df_clean.copy()['supplier'].apply(
        lambda x: unidecode(x))
    df_clean.loc[:, 'supplier_upper'] = df_clean['supplier_upper'].str.strip().str.upper()
    df_clean.loc[:, 'supplier_upper'] = df_clean['supplier_upper'].str.replace(
        '\t', '')
    df_clean.loc[:, 'supplier_upper'] = df_clean['supplier_upper'].str.replace(
        '\n', '')
    df_clean.loc[:, 'supplier_upper'] = df_clean['supplier_upper'].str.replace(
    ...
    print('Dropped duplicates worth £' + str(round((df['amount'].sum() - df_clean['amount'].sum()) /
                                                   1000000000, 2)) + 'bn.')
